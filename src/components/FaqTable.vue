...
const { filteredItems, faq,} = storeToRefs(faqStore);
const { activateEdit, activateDel, delId, editId } = faqStore;
...
<template>
    <div>
        <!-- <div class="overflow-auto rounded-lg shadow hidden md:block max-w-[100vw]">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="bg-white" v-for="item in filteredItems" :key="item.fee_component_id">
        ...
        <div class="flex flex-wrap  ">
            ...
            <div class="bg-white p-2 rounded-lg shadow m-1  smartphone:w-[100%] "
                v-for="item in filteredItems" :key="item.faq_id" v-motion-fade-visible-once>
                <div class="flex justify-end py-0">
                    <i class="fa-solid fa-x text-xs hover:cursor-pointer" @click="activateDel(item.faq_id)"></i>
                </div>
                <div>
                    <div class="flex items-center space-x-2 text-sm">
                        <div>
                            <span class="font-bold hover:underline">#{{ item.faq_id }}</span>
                        </div>
                        <div class="text-gray-500 font-bold ">{{ item.question}}</div>
                    </div>
                    <div class="flex">
                        <div class="text-sm text-gray-700 my-2">
                            <span class="bg-gray-100 p-1">{{ item.answer }} </span>
                        </div>
                ...
                <div class="flex justify-end py-0">
                    <i class="fa-regular fa-pen-to-square hover:cursor-pointer text-sm"
                        @click="activateEdit(item.faq_id, item.question,item.answer)"></i>
                </div>
            </div>
        </div>
    </div>

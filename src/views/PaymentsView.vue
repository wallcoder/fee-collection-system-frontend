...
import PaymentsPopups from '../components/PaymentsPopups.vue'
...
const { items,
    ...
    currentPage, } = storeToRefs(paymentStore);
const { getPayments } = paymentStore;
...
watch(search, (newValue) => {
    if (!newValue) {
        ...
        getPayments(newValue);
        filteredItems.value = items.value;
        console.log(items.value);
    } else {
        getPayments(false, 1, newValue);
        filteredItems.value = items.value.filter(payment =>
            payment.ref_no.toLowerCase().includes(newValue.toLowerCase()) ||
            payment.payment_type.toLowerCase().includes(newValue.toLowerCase()) ||
            payment.status.toLowerCase().includes(newValue.toLowerCase()) ||
            payment.payment_date.toLowerCase().includes(newValue.toLowerCase()) ||
            payment.stud_name.toLowerCase().includes(newValue.toLowerCase()) ||
            payment.reg_no.toLowerCase().includes(newValue.toLowerCase()) ||
            payment.payment_id.toString().includes(newValue) ||
            payment.amount.toString().includes(newValue)
        );
    }
});
        ...
        <div v-else>
            <div>
                ...
                <section class="flex flex-col items-center w-[100%] h-[85vh]  justify-center "
                    ...
                    <Empty title="payments" class="mb-2" v-motion-fade-visible-once/>
                ...
                <section v-else>
                    ...
                    <div class="w-full h-full" v-else-if="!loading && !error">
                        <section class="flex flex-col">
                            <h1 class="text-lg mb- pl-1">Payments</h1>
                            ...
                            <!-- TABLE -->
                            <PaymentsTable v-motion-fade-visible-once/>

                            
                        </section>

                        <!-- <PagiNav :totalPages="totalPages" :currentPage="currentPage" :onClick="getPayments"false,  /> -->
                                ...
                                <ul class="flex items-center -space-x-px h-8 text-sm">
                                    <li>
                                        <button :disabled="currentPage === 1"
                                            ...
                                            @click="getPayments(false, currentPage - 1)">
                                    ...
                                    <li v-for="page in totalPages" @click="getPayments(false, page)" :key="page">
                                        <a href="#"

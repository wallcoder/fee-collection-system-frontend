...
import StudentsPopups from '../components/StudentsPopups.vue'
...
const { items,
    ...
    currentPage, } = storeToRefs(studentStore);
...
watch(search, (newValue) => {
    if (!newValue) {
        ...
        getStudents(false, 1, newValue);
        filteredItems.value = items.value.filter(student =>
            student.reg_no.toLowerCase().includes(newValue.toLowerCase()) ||
            student.name.toLowerCase().includes(newValue.toLowerCase()) ||
            student.ph_no.toLowerCase().includes(newValue.toLowerCase()) ||
            student.roll_no.toLowerCase().includes(newValue.toLowerCase()) ||
            student.email.toLowerCase().includes(newValue.toLowerCase()) ||
            student.enrollment_year.toLowerCase().includes(newValue.toLowerCase()) ||
            student.student_id.toString().includes(newValue)
        );
    }
});
                ...
                <section v-else>
                    ...
                    <div class="w-full h-full" v-else-if="!loading && !error">
                        <section class="flex flex-col">
                            <h1 class="text-lg mb- pl-1">Students</h1>
                            ...
                            <!-- TABLE -->
                            <StudentsTable v-motion-fade-visible-once/>
                        ...
                        <!-- <PagiNav :totalPages="totalPages" :currentPage="currentPage" :onClick="getStudents" /> -->
                                ...
                                <ul class="flex items-center -space-x-px h-8 text-sm">
                                    <li>
                                        <button :disabled="currentPage === 1"
                                            ...
                                            @click="getStudents(false, currentPage - 1)">
                                    ...
                                    <li v-for="page in totalPages" @click="getStudents(false, page)" :key="page">
                                        <a href="#"
